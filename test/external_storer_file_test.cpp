...
class MockFileWriter : public FileHandlerInterface
{
  public:
    ...
    explicit MockFileWriter([[maybe_unused]] const std::string& baseDir) {}
...
class ExternalStorerFileWriterTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        fileWriter =
            std::make_unique<ExternalStorerFileWriter>(baseDir.string());
    }
...
TEST_F(ExternalStorerFileWriterTest, CreateFolderLeadingSlash)
{
    ...
    EXPECT_TRUE(std::filesystem::is_directory(baseDir / "valid_folder_leading_slash"));
}
...
TEST_F(ExternalStorerFileWriterTest, CreateFileLeadingSlash)
{
    ...
    EXPECT_TRUE(std::filesystem::exists(baseDir / "valid_file_leading_slash" /
                                        "index.json"));
}
