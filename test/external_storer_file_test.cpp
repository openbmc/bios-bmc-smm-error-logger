...
TEST_F(ExternalStorerFileTest, NoOdataTypeFailureTest)
{
    ...
    std::string jsonStr =
        "{\"@odata.id\":\"/redfish/v1/Systems/system/Memory/dimm0/MemoryMetrics\",\"Id\":\"Metrics\"}";
    EXPECT_THAT(exStorer->publishJson(jsonStr), false);
}
...
TEST_F(ExternalStorerFileTest, LogServiceNoIdTest)
{
    ...
    std::string jsonStr =
        "{\"@odata.id\":\"/redfish/v1/Systems/system/LogServices/6F7-C1A7C\",\"@odata.type\":\"#LogService.v1_1_0.LogService\"}";
    EXPECT_THAT(exStorer->publishJson(jsonStr), false);
}
...
TEST_F(ExternalStorerFileTest, LogServiceTest)
{
    ...
    std::string jsonStr =
        "{\"@odata.id\":\"/redfish/v1/Systems/system/LogServices/6F7-C1A7C\",\"@odata.type\":\"#LogService.v1_1_0.LogService\",\"Id\":\"6F7-C1A7C\"}";
...
TEST_F(ExternalStorerFileTest, LogEntryTest)
{
    ...
    // Now send a LogEntry
    std::string jsonLogEntry =
        "{\"@odata.id\":\"/some/odata/id\",\"@odata.type\":\"#LogEntry.v1_13_0.LogEntry\"}";
...
{
    // Try a other PDRs without @odata.id.
    std::string jsonStr =
        "\"@odata.type\":\"#MemoryMetrics.v1_4_1.MemoryMetrics\",\"Id\":\"Metrics\"}";
    EXPECT_THAT(exStorer->publishJson(jsonStr), false);
}
...
TEST_F(ExternalStorerFileTest, OtherSchemaTypeTest)
{
    ...
    std::string jsonStr =
        "{\"@odata.id\":\"/redfish/v1/Systems/system/Memory/dimm0/MemoryMetrics\",\"@odata.type\":\"#MemoryMetrics.v1_4_1.MemoryMetrics\",\"Id\":\"Metrics\"}";
    std::string exFolder =
