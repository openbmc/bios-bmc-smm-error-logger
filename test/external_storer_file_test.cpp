...
TEST_F(ExternalStorerFileTest, LogServiceTest)
{
    ...
    EXPECT_CALL(mockFileWriter, createFile(exServiceFolder, exServiceJson))
        .WillOnce(Return(true));
    EXPECT_CALL(mockFileWriter, createFile(exEntriesFolder, exEntriesJson))
        .WillOnce(Return(true));
...
TEST_F(ExternalStorerFileTest, LogEntryTest)
{
    ...
    EXPECT_CALL(mockFileWriter, createFile(_, _))
...
TEST_F(ExternalStorerFileTest, OtherSchemaTypeTest)
{
    ...
    EXPECT_CALL(mockFileWriter, createFile(exFolder, exJson))
        .WillOnce(Return(true));
    EXPECT_THAT(exStorer->publishJson(jsonStr), true);
}
