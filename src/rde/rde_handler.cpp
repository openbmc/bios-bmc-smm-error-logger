    ...
    RdeCommandHandler::operationInitRequest(std::span<const uint8_t> rdeCommand)
{
    ...
    {
        return RdeDecodeStatus::RdeNoError;
    }
    ...
    // Decoded the data.
    int bejRet =
        decoder.decode(dictionaries, std::span(encodedPldmBlock,
    ...
    RdeCommandHandler::multiPartReceiveResp(std::span<const uint8_t> rdeCommand)
{
    ...
    if (header->transferFlag == rdeMRecFlagStart)
    {
    ...
    else if (header->transferFlag == rdeMRecFlagMiddle)
    {
        ...
        {
            // Start of a new dictionary. Mark previous dictionary as complete.
    ...
    else if (header->transferFlag == rdeMRecFlagEnd)
    {
        if (flagState != RdeDictTransferFlagState::RdeStateStartRecvd)
        {
            fmt::print(
        ...
        // rdeCommand will have the DataIntegrityChecksum field. So omit that
        ...
        if (ret != RdeDecodeStatus::RdeNoError)
        {
            return ret;
        }
        return RdeDecodeStatus::RdeStopFlagReceived;
    }
    else if (header->transferFlag == rdeMRecFlagStartAndEnd)
    {
        // This is a beginning of a dictionary. Reset CRC.
        crc = 0xFFFFFFFF;
