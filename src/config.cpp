...
Config createConfig(std::string_view configPath)
{
    ...
    std::ifstream configJson(configPath.data());
    ...
    auto pollingIntervalMSInt = data.at("PollingIntervalMS");
    config.pollintIntervalMS = std::chrono::milliseconds(pollingIntervalMSInt);
